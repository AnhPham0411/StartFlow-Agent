...
To implement this design system, the frontend application uses the following stack and libraries (defined in `frontend/package.json`):

- **Core Framework**: Next.js 16 (App Router) + React 19.
- **Styling**: Vanilla CSS (CSS variables mapped to design tokens in `globals.css`). No utility classes (Tailwind) to ensure pure control over the editorial layout.
- **Data Visualization**: `recharts` (React charting library) for rendering the 12-month cash-flow and volatility trends.
- **Icons**: `lucide-react` for clean, lightweight system icons.
- **Authentication**: `keycloak-js` for secure OpenID Connect (OIDC) with PKCE flow.
- **Contracts / Schema Validation**: `zod` + `@startflow/contracts` for data contract parity between backend/frontend and AI service.
...
### 1. Progressive Disclosure (Tiết lộ Lũy tiến)
- **Rule**: Limit cognitive load. High-level decisions/summaries first. Technical details (agent trace logs, raw tool parameters, full JSON) must hide under interactive triggers (collapsible accordions, tabs, side drawers).
- **Design translation**: Accordions default to collapsed. Use `[Show Trace]` or metadata tabs for logs.

### 2. Path to Action / Next-Step Certainty (Hành trình khép kín)
- **Rule**: Never leave the user in a dead-end. When in an empty state, loading state, or error state, the UI must explicitly state how to recover or what to do next.
- **Design translation**:
  - Empty queue state shows: "Upload customer documents to begin assessment."
  - Low confidence warning shows: "Extraction confidence low. Click highlighted fields below to edit."
  - Processing error shows: "Agent timed out. Click 'Retry Run' or route manually to HITL."

### 3. Grounded Auditability (Minh bạch dữ liệu)
- **Rule**: AI assertions must be auditable in 1 click. If the system displays a credit/income score, the source data must be instantly accessible.
- **Design translation**: Hovering/clicking an extracted field highlights or scrolls to its source in the document viewer.

### 4. Interactive Semantics (Thẩm mỹ phục vụ Tiện ích)
- **Rule**: Visual indicators of high interest (e.g. warnings, low-confidence markings) must double as actions, not just static decoration.
- **Design translation**: Amber low-confidence fields are clickable edit inputs, not just colored text.
