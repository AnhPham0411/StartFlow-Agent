...
-- PostgreSQL 16 + pgvector · 27 bang · Sinh ngay 2026-07-18
-- ============================================================================
...
CREATE TYPE case_result       AS ENUM ('won','lost');
...
CREATE TABLE rag_cases (                              -- A5 RAG: cả case chốt LẪN trượt
    id                  BIGSERIAL PRIMARY KEY,
    embedding           vector(384),                  -- embedding cụm tag + ngữ cảnh (đã masking)
    tags                TEXT[] NOT NULL,
    product             product_enum NOT NULL,
    result              case_result NOT NULL,
    reason              TEXT,
    script_text_masked  TEXT NOT NULL,                -- KHÔNG PII (C5 nhóm 5)
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_rag_embedding ON rag_cases USING ivfflat (embedding vector_cosine_ops) WITH (lists = 10);
