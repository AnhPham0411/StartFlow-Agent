...
- Frontend: Angular 20 + `@sdcorejs/angular`, chỉ gọi NestJS API bằng access token.
...
1. Frontend đăng nhập qua realm/client public được cấu hình trong `frontend/src/environments/environment*.ts` theo Angular build configuration.
...
Các endpoint và profile seed được giữ trong lần hợp nhất Angular này. Các màn hình Sales Copilot React cũ không thuộc Angular runtime; việc port call list/customer/admin sang Angular Core UI là hạng mục frontend riêng trước khi bật journey NBA cho người dùng.
