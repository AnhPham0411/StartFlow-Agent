...
import { AlertTriangle, Check, ChevronDown, ChevronUp, Minus, TrendingDown, TrendingUp, X } from 'lucide-react';

import type { NbaAssessment, NbaCriterion, NbaPackageAssessment } from '@/src/lib/nba-assessment.types';
    ...
    <div className="stack">
      ...
      {blocks.map((c) => (
        <div key={c.code} className="banner banner--danger">
          ...
          <div>
            <p><strong>Chưa nên gọi khách này — {c.label.toLowerCase()}</strong></p>
            <p className="subtle" style={{ marginTop: 3 }}>
              {c.actual} · yêu cầu {c.required}. Các gói bên dưới vẫn được chấm đầy đủ để bạn nắm trước.
            </p>
          </div>
        </div>
      ))}
      ...
      {data.drift.length > 0 && (
        <div className="banner banner--warning">
          <AlertTriangle aria-hidden="true" />
          <div>
            <p><strong>{data.drift.length} tiêu chí đã đổi kể từ khi có đề xuất</strong></p>
        ...
        <PanelBody>
          ...
          {explanation.degraded_reason && (
            <div className="banner banner--info">
              <AlertTriangle aria-hidden="true" />
              <p>Chưa có câu mở lời: {explanation.degraded_reason}. Bảng tiêu chí bên dưới vẫn đầy đủ.</p>
            </div>
          )}
          ...
          {eligible.length === 0 ? (
            ...
            eligible.map((p) => (
              <PackageRow key={p.package} pkg={p} open={open === p.package} onToggle={() => toggle(p.package)} />
            ))
          )}
        ...
        <PanelBody>
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>
                ...
                <Row label="Trung bình mỗi tháng" a={String(w3.txn_per_month)} b={String(w6.txn_per_month)} />
                <Row label="Giao dịch lớn nhất" a={short(w3.max_txn_amount)} b={short(w6.max_txn_amount)} />
                <Row label="Tháng có phát sinh" a={`${w3.active_months}/3`} b={`${w6.active_months}/6`} />
              </tbody>
            </table>
          </div>
          ...
          {trend.net_flow_pct !== null && (
            <p className={`trend trend--${trend.direction === 'flat' ? 'up' : trend.direction}`}>
              {trend.direction === 'up' ? <TrendingUp size={16} /> : trend.direction === 'down' ? <TrendingDown size={16} /> : <Minus size={16} />}
          ...
          {w3.spend_tags.length > 0 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div className="chip-row">
                {w3.spend_tags.map((t) => (
                  <span key={t.tag} className="chip">{t.tag} · {t.txn_count}</span>
                ))}
              </div>
            </div>
          )}
    ...
    <Panel>
      ...
      <PanelBody>
        <div className="stat-grid">
          ...
          <Stat label="Nợ / thu nhập" value={current.dti === null ? 'chưa xác định' : `${(current.dti * 100).toFixed(1)}%`} />
          <Stat label="Nhóm CIC" value={current.cic_group === null ? '—' : `Nhóm ${current.cic_group}`} />
          <Stat label="Tuổi" value={current.age === null ? '—' : String(current.age)} />
          <Stat label="Khoản quá hạn" value={current.has_overdue ? 'Có' : 'Không'} danger={current.has_overdue} />
        </div>

        <p className="subtle" style={{ marginTop: 12, marginBottom: 0 }}>
          Số dư và dư nợ là giá trị hiện tại — hệ thống chưa lưu lịch sử số dư nên không tính
          trung bình theo tháng được.
        </p>
        ...
        <div className="stack-sm">
          <div>
            ...
            {relationship.held_products.length === 0 ? (
              <p className="muted" style={{ margin: 0, fontSize: '0.86rem' }}>Chưa có sản phẩm nào</p>
            ) : (
              <div className="chip-row">
                {relationship.held_products.map((h, i) => (
                  <span key={i} className="chip">
                    {PROD[h.product] ?? h.product}{h.tier ? ` · ${h.tier}` : ''}
                  </span>
                ))}
              </div>
            )}
            ...
            <div>
              ...
              <div className="chip-row">
                {relationship.suppressed.map((s, i) => (
                  <span key={i} className="chip" style={{ color: 'var(--danger)', borderColor: 'var(--danger-line)', background: 'var(--danger-soft)' }}>
...
function Row({ label, a, b, strong }: { label: string; a: string; b: string; strong?: boolean }) {
  return (
    <tr>
      <td>{label}</td>
      <td className="num" style={{ textAlign: 'right', fontWeight: strong ? 700 : 500 }}>{a}</td>
      <td className="num muted" style={{ textAlign: 'right' }}>{b}</td>
    </tr>
  );
}
...
function PackageRow({ pkg, open, onToggle }: { pkg: NbaPackageAssessment; open: boolean; onToggle: () => void }) {
      ...
      <button type="button" className="pkg__toggle" onClick={onToggle} aria-expanded={open}>
        ...
        <span className="pkg__name">
          ...
          <span className="pkg__meta">
            ...
            {pkg.tier ? ` · ${pkg.tier}` : ''}
            {!pkg.eligible && pkg.blocked_by.length > 0 && ` · chặn bởi ${pkg.blocked_by.join(', ')}`}
          </span>
        </span>
        {pkg.multiplier !== 1 && <Badge tone={pkg.multiplier > 1 ? 'success' : 'warning'}>×{pkg.multiplier}</Badge>}
        <span className="pkg__count">{passed}/{pkg.criteria.length}</span>
        {open ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
      </button>

      {open && (
        <div className="pkg__criteria">
          {pkg.criteria.map((c) => <Criterion key={c.code} c={c} />)}
        </div>
      )}
    </div>
  );
    ...
    <div className={`criterion criterion--${tone}`}>
      ...
      <div>
        ...
        <span className="criterion__code">
          {c.code} · {c.source === 'catalog' ? 'điều kiện sản phẩm' : 'rule'}{!c.blocking && ' · không chặn'}
        </span>
