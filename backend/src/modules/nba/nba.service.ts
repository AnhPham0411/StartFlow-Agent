...
export class NbaService {
  ...
  async getCustomer(customerId: number, user: AuthenticatedUser): Promise<CustomerDetail> {
    ...
    const [cust] = await this.prisma.$queryRawUnsafe<
      Array<{ full_name: string; cif_code: string }>
    >(`SELECT full_name, cif_code FROM customers WHERE id = $1::bigint`, customerId);
  ...
  async submitFeedback(
    ...
    await this._assertCanAccessCustomer(rec.customer_id, user);
    ...
    await this._audit(user, 'create', 'feedback', body.rec_id, {
  ...
  async assignCallList(
    ...
    if (!isAdmin(user)) {
      ...
      const [scope] = await this.prisma.$queryRawUnsafe<Array<{ n: number }>>(
