 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * CẢNH BÁO TRÙNG LẶP LOGIC
 * Bộ rule R1..R12 gốc nằm ở `apps/ai/src/ranker/rules.py`; ngưỡng gốc ở
 * `apps/ai/src/core/config.py` (BUILD_SPEC L8 yêu cầu tập trung tham số ở đó).
 * File này là BẢN SAO bằng TypeScript theo yêu cầu "viết ở BE". Hai bản CÓ THỂ LỆCH NHAU.
 * Sửa rule hay ngưỡng ở một bên thì BẮT BUỘC sửa bên còn lại.
 * Ngưỡng gom hết vào RULE_PARAMS bên dưới để chỗ cần đồng bộ là duy nhất.
 * Kiểm tra đồng bộ: backend/test/policy-parity.spec.ts
 * ─────────────────────────────────────────────────────────────────────────────
 *
...
/** Phải khớp apps/ai/src/core/config.py + rules.py. Xem cảnh báo đầu file. */
...
const money = (v: number | null): string => (v === null ? 'không có dữ liệu' : `${VND.format(v)} đ`);
  ...
  private evaluatePackage(
    ...
    if (R3_PRODUCTS.includes(row.product)) {
      add({
        ...
        actual:
          current.dti === null
            ? 'chưa có thu nhập để tính'
            : `${(current.dti * 100).toFixed(1)}%`,
